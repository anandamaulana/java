 ...
 * (Year-over-Year): X = urutan tahun ke-n untuk bulan yang sama, Y = total omzet bulan tersebut.
 * Y = a + bX, dengan:
...
public class RegresiLinearService {
    ...
    public PrediksiResult prediksi(int bulanTarget, int tahunTarget) throws SQLException {
        List<OmzetBulanan> historis = transaksiDAO.getOmzetByBulan(bulanTarget, tahunTarget);
        int n = historis.size();
        if (n < 2) {
            throw new IllegalStateException("Data historis bulan ini sebelum tahun " + tahunTarget +
                    ...
                    "Lengkapi data lewat menu Import/Kelola Data Transaksi lalu Rekap ke Omzet Bulanan.");
        }
